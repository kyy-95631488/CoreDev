...
function EditTeamMemberContent() {
      ...
      const fetchMemberData = async () => {
        try {
          ...
          const response = await fetch(
            `https://hendriansyah.xyz/v1/auth/get-team-members-list/?session_token=${encodeURIComponent(sessionToken)}`,
            {
              method: 'GET',
              headers: { 'Content-Type': 'application/json' },
            }
          ...
          const data = await response.json();
          if (response.ok) {
            const member = data.team_members.find((m) => m.email === memberEmail);
            if (member) {
              setFormData({
                name: member.name || '',
                email: member.email || '',
                ...
                portfolioLink: member.portfolio_link || '',
                photo: null,
            ...
            setDialogState({
              isOpen: true,
              title: 'Error',
              message: `Failed to fetch team member: ${data.error}`,
              isConfirm: false,
            });
          }
        } catch (err) {
  ...
  const validateForm = () => {
    ...
    if (!formData.photo && !formData.photo_url) newErrors.photo = 'Photo is required';
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setPreviewImage(formData.photo_url || null);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://hendriansyah.xyz/v1/auth/update-team-member/', {
        method: 'PUT',
        body: formDataToSend,
      });
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        setDialogState({
          isOpen: true,
          title: 'Error',
          message: `Failed to update team member: ${data.error || 'Unknown error'}`,
          isConfirm: false,
          onConfirm: null,
        });
      }
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" encType="multipart/form-data">
          ...
          >
            <motion.button
              ...
              className={`w-full px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-400 text-white rounded-lg font-semibold text-sm flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              <Pencil size={18} />
